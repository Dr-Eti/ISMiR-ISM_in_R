...
The code explores surprising, but unnoticed similarities with other well-known algorithms. Namely, the code includes a function for finding strongly connected components (SCC) by Depth-First Search, which is borrowed from my other repository (https://github.com/Dr-Eti/RtarD-Tarjans_DFS_in_R.git).  It also provides a block-triangular permutation of the corresponding adjacency matrix.
